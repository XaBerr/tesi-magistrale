...
As shown from \fig{measure/PID_on_off} the drift was compensated by moving the centroid from $\vec{m}_{off} = (0.21, -0.04)$ to $\vec{m}_{on} = (-0.01, 0.03)$ mm, while fluctuations have worsened as seen by comparing standard deviation from $\vec{\sigma}_{off} = (0.10, 0.08)$ to $\vec{\sigma}_{on} = (0.09, 0.30)$ mm.
