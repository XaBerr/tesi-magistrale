...
In the first figure~\fig{measure/coarse_off} where both PATC and PATF are disabled, it can be seen that the Bob's centroid starts to accumulate drift in the x-axis that leads it to lose the signal at about 7 seconds.
...
In this figure it is clearly visible that the One shot of the PATC has compensated the drift that before had carried to the loss of the signal. This is a similar result to the second test \textit{One shot on/off with heat source turbolence}.
...
When the system is turned on the fluctuations and drifts are compensated similarly as already seen in the first test \textit{PIDKPA101 on/off with heat source turbolence} but in this experiment it is important to note that the PATF system could only operate thanks to the PATC compensation.

%TODO: cacolare le palline del punto 3
